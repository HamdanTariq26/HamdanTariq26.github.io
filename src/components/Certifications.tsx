...
export default function Certifications() {
  ...
  return (
    <section
      id="certifications"
      className="scroll-mt-14 border-b border-border bg-page py-20 lg:py-24"
    >
              ...
              <div className="relative overflow-hidden rounded-xl border-2 border-accent/40 bg-white p-6 sm:p-8 shadow-sm transition-all duration-300 hover:border-accent hover:shadow-lg hover:-translate-y-0.5">
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-3 font-[family-name:var(--font-display)] text-xl font-bold text-text-primary sm:text-2xl">
                      {hcia.title}
                    </h3>
                ...
                <div className="mt-6 flex flex-col gap-4 border-t border-border pt-4 sm:flex-row sm:items-center sm:justify-between">
                  {hcia.signatories && (
                    <div className="text-xs text-text-muted space-y-0.5">
                      <span className="font-semibold uppercase tracking-wider text-[10px] text-text-dim">
                        Instructors:
                      </span>
                ...
                <div className="mt-6 flex flex-col gap-4 border-t border-border pt-4 sm:flex-row sm:items-center sm:justify-between">
                  {item.referenceId ? (
                    <div className="text-xs text-text-muted font-mono">
                      <span>Ref: </span>
                      <span className="text-text-secondary">{item.referenceId}</span>
                    </div>
                  ) : <div />}
          ...
          <div className="mt-8">
            {outreachActivities.map((act) => (
              <AnimateIn key={act.id} animation="fade-up" delay={150}>
                <div className="relative overflow-hidden rounded-xl border-2 border-accent/40 bg-white p-6 sm:p-7 shadow-sm transition-all duration-300 hover:border-accent hover:shadow-lg hover:-translate-y-0.5">
                  {/* Blue top accent bar */}
                  <div className="absolute top-0 left-0 right-0 h-1 bg-accent" />
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      ...
                      <h4 className="mt-3 font-[family-name:var(--font-display)] text-base font-bold text-text-primary sm:text-lg">
                        {act.title}
                      </h4>
                  ...
                  <div className="mt-5 flex items-center justify-end border-t border-border pt-3.5">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center justify-center gap-1.5 flex-shrink-0 whitespace-nowrap rounded-lg border border-border bg-page-subtle px-3.5 py-1.5 text-xs font-medium text-text-secondary transition-all duration-200 hover:border-accent-border hover:bg-accent-subtle hover:text-accent hover:shadow-sm active:scale-95"
                    >
                      <span>View Certificate</span>
                      <IconExternalLink className="h-3.5 w-3.5" />
                    </a>
